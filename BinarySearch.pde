...
public int binarySearch(int catNumToFind)
{
  ...
  while (max>=min) {
    ...
    else if (store[mid].getCatNum()>catNumToFind) max = mid-1;
    else min = mid+1;
  }   
  return -1;
}
public int recursiveBinarySearch(int catNumToFind, int nLow, int nHigh)
{
  ...
  else if (store[mid].getCatNum() == catNumToFind) return store[mid].getInventory();
  else if (store[mid].getCatNum() > catNumToFind) return recursiveBinarySearch(catNumToFind, nLow, mid-1);
  else return recursiveBinarySearch(catNumToFind, nLow+1, nHigh);
}
public void setup()
{
